...
vec3 mkdot(vec3 bg, vec2 place, float radius, vec3 color, vec2 uv) {
	...
	//float pct      = distance(uv.x, place.x) + distance(uv.y, place.y);
	//float pct      = distance(uv.x, place.x) * distance(uv.y, place.y);
	float pct        = min(distance(uv.x, place.x), distance(uv.y, place.y));
	//float pct      = max(distance(uv.x, place.x), distance(uv.y, place.y));
	//float pct      = pow(distance(uv.x, place.x), distance(uv.y, place.y));
