...
void main() {
  ...
  vec2 id = floor(uv);
  float checkered_board = mod(id.x+id.y, 2.0);

  // color.r += checkered_board * 0.3;
  ...
  if (checkered_board == 1.0) {
    uv = uv.yx;
  }

  float imperfection = 0.2;
  float t = (pendulum -0.5) *(0.5 - imperfection) + 0.25;
